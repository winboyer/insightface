...
if __name__ == '__main__':
    ...
    detector = SCRFD(model_file='/root/jinyfeng/models/scrfd-models/antelopev2/scrfd_10g_bnkps.onnx')
    detector.prepare(-1)
#     img_paths = ['tests/data/t3.jpg']
    root_dir = '/root/jinyfeng/datas/sensoro'
    img_folder = os.path.join(root_dir, 'sensoro_person_info')
    save_folder = os.path.join(root_dir, 'sensoro_person_info_cropface_scrfd')
    facedet_result = os.path.join(root_dir, 'sensoro_person_info_bbox_ldmark_scrfd.txt')
...
#         print('img_path, bboxes.shape===========', img_path, bboxes.shape)
#         if kpss is not None:
#             print(kpss.shape)
...
#             x1,y1,x2,y2,_ = bbox.astype(np.int32)
            ...
            if kpss is not None:
                kps = kpss[i].astype(np.int)
                for l in range(kps.shape[0]):
                    if l<4:
                        linestr += str(kps[l][0])+','+str(kps[l][1])+','
                    else:
                        linestr += str(kps[l][0])+','+str(kps[l][1])+'\n'
